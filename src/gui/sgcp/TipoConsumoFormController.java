...
public class TipoConsumoFormController implements Initializable {

	private TipoConsumo entity;
...
// lan�a Erros - nome do cpo e msg de erro
		if (textNomeTipo.getText() == null || textNomeTipo.getText().trim().contentEquals("")) {
			exception.addErros("descrição", "Descrição é obrigatório");
		}
		else {
			if (textNomeTipo.getText().length() < 5) {
				exception.addErros("descrição", "Descrição é inválida");
			}
		}
 ...
 */
 ...
 // mandando a msg de erro para o labelErro correspondente 	
 	private void setErrorMessages(Map<String, String> erros) {
 		...
 		labelErrorNomeTipo.setText((fields.contains("descrição") ? erros.get("descrição") : ""));		
	}
}	
